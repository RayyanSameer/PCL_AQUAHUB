...
### Vendor Dashboard (local simulation)
A standalone vendor management simulation using SQLite + Express + React is included in `src/vendor-dashboard/`. It runs independently of the Flask backend and is useful for testing vendor workflows locally.
...
npm install
npm run db:setup    # creates aquahub_vendor.db and seeds mock data
...
Default vendor credentials (seed data): `john@vendor.com` / `vendor123`

## API Endpoints

- `POST /api/register/customer` - Register new customer
- `POST /api/register/vendor` - Register new vendor
- `GET /api/users` - (dev) list users
- `GET /api/health` - health check and DB connectivity (useful for smoke tests)
